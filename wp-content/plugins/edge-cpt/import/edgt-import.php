...
class CoyoteEdgeImport {
    ...
    function init_edgt_import() {
        if(isset($_REQUEST['import_option'])) {
            ...
            if($import_option == 'content'){
                $this->import_content('coyote_content.xml');
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_coyote', $options);
    ...
    function edgt_admin_import() {
        if (edgt_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($coyote_edge_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'coyote_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'coyote_edge_enqueue_admin_styles');
...
function edgt_init_import_object(){
    global $coyote_edge_import_object;
    $coyote_edge_import_object = new CoyoteEdgeImport();
}

add_action('init', 'edgt_init_import_object');

if(!function_exists('coyote_edge_dataImport')){
    function coyote_edge_dataImport(){
        global $coyote_edge_import_object;

        if ($_POST['import_attachments'] == 1)
            $coyote_edge_import_object->attachments = true;
        else
            $coyote_edge_import_object->attachments = false;

        $folder = "coyote/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $coyote_edge_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_edgt_dataImport', 'coyote_edge_dataImport');
}

if(!function_exists('coyote_edge_widgetsImport')){
    function coyote_edge_widgetsImport(){
        global $coyote_edge_import_object;
        ...
        $coyote_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_edgt_widgetsImport', 'coyote_edge_widgetsImport');
}

if(!function_exists('coyote_edge_optionsImport')){
    function coyote_edge_optionsImport(){
        global $coyote_edge_import_object;
        ...
        $coyote_edge_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_edgt_optionsImport', 'coyote_edge_optionsImport');
}

if(!function_exists('coyote_edge_otherImport')){
    function coyote_edge_otherImport(){
        global $coyote_edge_import_object;
        ...
        $coyote_edge_import_object->import_options($folder.'options.txt');
        $coyote_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $coyote_edge_import_object->import_menus($folder.'menus.txt');
        $coyote_edge_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_edgt_otherImport', 'coyote_edge_otherImport');
}