 ...
 */
if (!class_exists('CoyoteEdgeStickyNavigationWalker')) {
	class CoyoteEdgeStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if (coyote_edge_options()->getOptionValue('enable_wide_menu_background') == "yes"){
			...
			if($item->hide == ""){
                ...
                if($item->featured_icon !== ""){
                    $item_output .= '<span class="edgtf-featured-icon '. $item->featured_icon .'"></span>';
                }
                
				//append arrow for dropdown
				if($args->has_children && $depth == 1) {
					$item_output .= '<i class="edgtf-menu-arrow edgtf-icon-linea-icon icon-arrows-plus"></i>';
				}
