...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(coyote_edge_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($back_to_top_skin === 'light'){
    $css_classes[] = 'edgtf-btt-'.$back_to_top_skin.'-skin';
}
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.coyote_edge_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="edgtf-mobile-video-image" ' . coyote_edge_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	$video_output .= '<div ' . coyote_edge_get_class_attribute($video_overlay_class) . coyote_edge_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="edgtf-video-wrap">';
	$video_output .= '<video class="edgtf-video" width="1920" height="800" '. coyote_edge_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . coyote_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','coyote').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
