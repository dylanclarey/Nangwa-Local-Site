<?php

if ( ! function_exists('coyote_edge_sidebar_options_map') ) {

	function coyote_edge_sidebar_options_map() {

		coyote_edge_add_admin_page(
			array(
				'slug'  => '_sidebar_page',
				'title' => esc_html__('Sidebar', 'coyote'),
				'icon'  => 'fa fa-indent'
			)
		);

		$panel_widgets = coyote_edge_add_admin_panel(
			array(
				'page'  => '_sidebar_page',
				'name'  => 'panel_widgets',
				'title' => esc_html__('Widgets', 'coyote'),
			)
		);

		/**
		 * Navigation style
		 */
		coyote_edge_add_admin_field(array(
			'type'			=> 'color',
			'name'			=> 'sidebar_background_color',
			'default_value'	=> '',
			'label'			=> esc_html__('Sidebar Background Color', 'coyote'),
			'description'	=> esc_html__('Choose background color for sidebar', 'coyote'),
			'parent'		=> $panel_widgets
		));

		$group_sidebar_padding = coyote_edge_add_admin_group(array(
			'name'		=> 'group_sidebar_padding',
			'title'		=> esc_html__('Padding', 'coyote'),
			'parent'	=> $panel_widgets
		));

		$row_sidebar_padding = coyote_edge_add_admin_row(array(
			'name'		=> 'row_sidebar_padding',
			'parent'	=> $group_sidebar_padding
		));

		coyote_edge_add_admin_field(array(
			'type'			=> 'textsimple',
			'name'			=> 'sidebar_padding_top',
			'default_value'	=> '',
			'label'			=> esc_html__('Top Padding', 'coyote'),
			'args'			=> array(
				'suffix'	=> 'px'
			),
			'parent'		=> $row_sidebar_padding
		));

		coyote_edge_add_admin_field(array(
			'type'			=> 'textsimple',
			'name'			=> 'sidebar_padding_right',
			'default_value'	=> '',
			'label'			=> esc_html__('Right Padding', 'coyote'),
			'args'			=> array(
				'suffix'	=> 'px'
			),
			'parent'		=> $row_sidebar_padding
		));

		coyote_edge_add_admin_field(array(
			'type'			=> 'textsimple',
			'name'			=> 'sidebar_padding_bottom',
			'default_value'	=> '',
			'label'			=> esc_html__('Bottom Padding', 'coyote'),
			'args'			=> array(
				'suffix'	=> 'px'
			),
			'parent'		=> $row_sidebar_padding
		));

		coyote_edge_add_admin_field(array(
			'type'			=> 'textsimple',
			'name'			=> 'sidebar_padding_left',
			'default_value'	=> '',
			'label'			=> esc_html__('Left Padding', 'coyote'),
			'args'			=> array(
				'suffix'	=> 'px'
			),
			'parent'		=> $row_sidebar_padding
		));

		coyote_edge_add_admin_field(array(
			'type'			=> 'select',
			'name'			=> 'sidebar_alignment',
			'default_value'	=> '',
			'label'			=> esc_html__('Text Alignment', 'coyote'),
			'description'	=> esc_html__('Choose text aligment', 'coyote'),
			'options'		=> array(
				'left' => esc_html__('Left', 'coyote'),
				'center' => esc_html__('Center', 'coyote'),
				'right' => esc_html__('Right', 'coyote'),
			),
			'parent'		=> $panel_widgets
		));

	}

	add_action( 'coyote_edge_options_map', 'coyote_edge_sidebar_options_map', 9);

}