...
class EdgeSkin extends CoyoteEdgeSkinAbstract {
    /**
     * Skin constructor. Hooks to coyote_edge_admin_scripts_init and coyote_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('coyote_edge_admin_scripts_init', array($this, 'registerStyles'));
        add_action('coyote_edge_admin_scripts_init', array($this, 'registerScripts'));

        add_action('coyote_edge_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('coyote_edge_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('coyote_edge_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('coyote_edge_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            coyote_edge_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            coyote_edge_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = coyote_edge_get_admin_tab();
        $active_page = $coyote_edge_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param CoyoteEdgeAdminPage $page current page to load
     *
     * @see EdgeSkinAbstract::loadTemplatePart()
     */
