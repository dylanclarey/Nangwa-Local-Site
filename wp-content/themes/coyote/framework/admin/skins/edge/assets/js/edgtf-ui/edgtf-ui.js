(function($){
	...
	function edgtChangedInput () {
		$('.edgtf-tabs-content form.edgt_ajax_form:not(.edgtf-import-page-holder):not(.edgtf-backup-options-page-holder)').on('change keyup keydown', 'input:not([type="submit"]), textarea, select', function (e) {
			$('.edgtf-input-change').addClass('yes');
		});

        $('.edgtf-tabs-content form.edgt_ajax_form:not(.edgtf-import-page-holder):not(.edgtf-backup-options-page-holder) .field.switch label:not(.selected)').click( function() {
            $('.edgtf-input-change').addClass('yes');
        });

        $('.edgtf-tabs-content form.edgt_ajax_form:not(.edgtf-import-page-holder):not(.edgtf-backup-options-page-holder) #anchornav input').click(function() {
			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=coyote_edge_gallery_upload_get_images&ids=" + ids,
    ...
    */
	...
	function edgtfInitAjaxForm() {
		...
		$(document).delegate(".edgt_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "coyote_edge_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
		...
		if($('.edgtf-backup-options-page-holder').length) {
			...
			edgtfImportBtn.on('click', function(e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'coyote_edge_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
					$.ajax({
