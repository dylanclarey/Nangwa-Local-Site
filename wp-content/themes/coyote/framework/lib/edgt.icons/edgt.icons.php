...
/*
  Class: CoyoteEdgeIconCollections
...
class CoyoteEdgeIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new CoyoteEdgeIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new CoyoteEdgeIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new CoyoteEdgeIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new CoyoteEdgeLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new CoyoteEdgeLinearIcons('Linear Icons', 'linear_icons'));
    }
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon pack','coyote'),
        ...
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon','coyote'),
    ...
    public function getSocialIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack','coyote'),
            'name'    => 'icon_pack',
            'options' => array(
    ...
    public function getIconSizesArray() {
        return array(
            esc_html__("Tiny",'coyote')       => "fa-lg",
            esc_html__("Small",'coyote')      => "fa-2x",
            esc_html__("Medium",'coyote')     => "fa-3x",
            esc_html__("Large",'coyote')      => "fa-4x",
            esc_html__("Very Large",'coyote') => "fa-5x"
        );
    }
    ...
    public function getIconCollectionIcons(iCoyoteEdgeIconCollection $collection) {
...
if ( ! function_exists( 'coyote_edge_activate_theme_icons' ) ) {

	function coyote_edge_activate_theme_icons() {
		global $coyote_edge_IconCollections;
		$coyote_edge_IconCollections = CoyoteEdgeIconCollections::get_instance();
	}

	add_action('after_setup_theme', 'coyote_edge_activate_theme_icons');
}