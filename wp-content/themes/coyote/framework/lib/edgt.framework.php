...
class CoyoteEdgeFramework {
    ...
    private function __construct() {
        $this->edgtOptions = CoyoteEdgeOptions::get_instance();
        $this->edgtMetaBoxes = CoyoteEdgeMetaBoxes::get_instance();
    }
	...
	public function setSkin(CoyoteEdgeSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class CoyoteEdgeSkinManager
 *
...
/**
 * Class CoyoteEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class CoyoteEdgeSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(coyote_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
		...
		return coyote_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png';
	}
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.edgtSCIcon = '<?php echo coyote_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class CoyoteEdgeOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $coyote_edge_options)) {
            return $coyote_edge_options[$key];
...
/*
   Class: CoyoteEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class CoyoteEdgeAdminPage implements iCoyoteEdgeLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new CoyoteEdgeFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iCoyoteEdgeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: CoyoteEdgeMetaBoxes
...
/*
   Class: CoyoteEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class CoyoteEdgeMetaBox implements iCoyoteEdgeLayoutNode {
	...
	public function setTitle($label) {
		...
		return $coyote_edge_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'coyote_edge_init_framework_variable' ) ) {

	function coyote_edge_init_framework_variable() {

		global $coyote_edge_Framework;

		$coyote_edge_Framework = CoyoteEdgeFramework::get_instance();
		$edgtSkinManager = new CoyoteEdgeSkinManager();
		$coyote_edge_Framework->setSkin($edgtSkinManager->getSkin());
	}

	add_action('coyote_edge_before_options_map', 'coyote_edge_init_framework_variable' );
}
?>