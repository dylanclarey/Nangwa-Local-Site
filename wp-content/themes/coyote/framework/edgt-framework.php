...
if(!function_exists('coyote_edge_admin_scripts_init')) {
	/**
	 * Function that registers all scripts that are necessary for our back-end
	 */
	function coyote_edge_admin_scripts_init() {

        /**
         * @see EdgeSkinAbstract::registerScripts - hooked with 10
         * @see EdgeSkinAbstract::registerStyles - hooked with 10
         */
        do_action('coyote_edge_admin_scripts_init');
	}

	add_action('admin_init', 'coyote_edge_admin_scripts_init');
}

if(!function_exists('coyote_edge_enqueue_admin_styles')) {
	/**
	 * Function that enqueues styles for options page
	 */
	function coyote_edge_enqueue_admin_styles() {
		wp_enqueue_style('wp-color-picker');

        /**
         * @see EdgeSkinAbstract::enqueueStyles - hooked with 10
         */
        do_action('coyote_edge_enqueue_admin_styles');
	}
}

if(!function_exists('coyote_edge_enqueue_admin_scripts')) {
	/**
	 * Function that enqueues styles for options page
	 */
	function coyote_edge_enqueue_admin_scripts() {
         ...
         */
        do_action('coyote_edge_enqueue_admin_scripts');
	}
}

if(!function_exists('coyote_edge_enqueue_meta_box_styles')) {
	/**
	 * Function that enqueues styles for meta boxes
	 */
	function coyote_edge_enqueue_meta_box_styles() {
		wp_enqueue_style( 'wp-color-picker' );

        /**
         * @see EdgeSkinAbstract::enqueueStyles - hooked with 10
         */
        do_action('coyote_edge_enqueue_meta_box_styles');
	}
}

if(!function_exists('coyote_edge_enqueue_meta_box_scripts')) {
	/**
	 * Function that enqueues scripts for meta boxes
	 */
	function coyote_edge_enqueue_meta_box_scripts() {
		wp_enqueue_script('wp-color-picker');
		wp_enqueue_script('jquery-ui-datepicker');
		wp_enqueue_script('jquery-ui-accordion');
		wp_enqueue_script('common');
         ...
         * @see EdgeSkinAbstract::enqueueScripts - hooked with 10
         */
        do_action('coyote_edge_enqueue_meta_box_scripts');
	}
}

if(!function_exists('coyote_edge_enqueue_nav_menu_script')) {
	/**
	 * Function that enqueues styles and scripts necessary for menu administration page.
	 * It checks $hook variable
	 * @param $hook string current page hook to check
	 */
	function coyote_edge_enqueue_nav_menu_script($hook) {
	...
	add_action('admin_enqueue_scripts', 'coyote_edge_enqueue_nav_menu_script');
}


if(!function_exists('coyote_edge_enqueue_widgets_admin_script')) {
	/**
	 * Function that enqueues styles and scripts for admin widgets page.
	 * @param $hook string current page hook to check
	 */
	function coyote_edge_enqueue_widgets_admin_script($hook) {
		if($hook == 'widgets.php') {
			wp_enqueue_script('edgtf-dependence');
		}
	}

	add_action('admin_enqueue_scripts', 'coyote_edge_enqueue_widgets_admin_script');
}


if(!function_exists('coyote_edge_enqueue_styles_slider_taxonomy')) {
	/**
	 * Enqueue styles when on slider taxonomy page in admin
	 */
	function coyote_edge_enqueue_styles_slider_taxonomy() {
		if(isset($_GET['taxonomy']) && $_GET['taxonomy'] == 'slides_category') {
			coyote_edge_enqueue_admin_styles();
		}
	}

	add_action('admin_print_scripts-edit-tags.php', 'coyote_edge_enqueue_styles_slider_taxonomy');
}

if(!function_exists('coyote_edge_init_theme_options_array')) {
	/**
	 * Function that merges $coyote_edge_options and default options array into one array.
	 *
	 * @see array_merge()
	 */
	function coyote_edge_init_theme_options_array() {
        global $coyote_edge_options, $coyote_edge_Framework;

		$db_options = get_option('edgt_options_coyote');

		//does edgt_options exists in db?
		if(is_array($db_options)) {
			//merge with default options
			$coyote_edge_options  = array_merge($coyote_edge_Framework->edgtOptions->options, get_option('edgt_options_coyote'));
		} else {
			//options don't exists in db, take default ones
			$coyote_edge_options = $coyote_edge_Framework->edgtOptions->options;
		}
	}

	add_action('coyote_edge_after_options_map', 'coyote_edge_init_theme_options_array', 0);
}

if(!function_exists('coyote_edge_init_theme_options')) {
	/**
	 * Function that sets $coyote_edge_options variable if it does'nt exists
	 */
	function coyote_edge_init_theme_options() {
		global $coyote_edge_options;
		global $coyote_edge_Framework;
		if(isset($coyote_edge_options['reset_to_defaults'])) {
			if( $coyote_edge_options['reset_to_defaults'] == 'yes' ) delete_option( "edgt_options_coyote");
		}

		if (!get_option("edgt_options_coyote")) {
			add_option( "edgt_options_coyote",
				$coyote_edge_Framework->edgtOptions->options
			);
...
if(!function_exists('coyote_edge_register_theme_settings')) {
	/**
	 * Function that registers setting that will be used to store theme options
	 */
	function coyote_edge_register_theme_settings() {
		register_setting( 'coyote_edge_theme_menu', 'edgt_options' );
	}

	add_action('admin_init', 'coyote_edge_register_theme_settings');
}

if(!function_exists('coyote_edge_get_admin_tab')) {
	/**
	 * Helper function that returns current tab from url.
	 * @return null
	 */
	function coyote_edge_get_admin_tab(){
		return isset($_GET['page']) ? coyote_edge_strafter($_GET['page'],'tab') : NULL;
	}
}

if(!function_exists('coyote_edge_strafter')) {
	/**
	 ...
	 */
	function coyote_edge_strafter($string, $substring) {
...
if(!function_exists('coyote_edge_save_options')) {
	/**
	 * Function that saves theme options to db.
	 * It hooks to ajax wp_ajax_edgtf_save_options action.
	 */
	function coyote_edge_save_options() {
		global $coyote_edge_options;
		if(current_user_can('administrator')) {
			...
			$coyote_edge_options = array_merge($coyote_edge_options, $_REQUEST);

			update_option('edgt_options_coyote', $coyote_edge_options);

			do_action('coyote_edge_after_theme_option_save');
			echo "Saved";
		}
		die();
	}

	add_action('wp_ajax_coyote_edge_save_options', 'coyote_edge_save_options');
}

if(!function_exists('coyote_edge_meta_box_add')) {
	/**
	 * Function that adds all defined meta boxes.
	 * It loops through array of created meta boxes and adds them
	 */
	function coyote_edge_meta_box_add() {
		global $coyote_edge_Framework;


		foreach ($coyote_edge_Framework->edgtMetaBoxes->metaBoxes as $key=>$box ) {
			$hidden = false;
			if (!empty($box->hidden_property)) {
				foreach ($box->hidden_values as $value) {
					if (coyote_edge_option_get_value($box->hidden_property)==$value)
						$hidden = true;
			...
			if(is_array($box->scope) && count($box->scope)) {
				foreach($box->scope as $screen) {
					...
					if ($hidden) {
						add_filter( 'postbox_classes_'.$screen.'_edgtf-meta-box-'.$key, 'coyote_edge_meta_box_add_hidden_class');
					}
				}
			}
		...
		add_action('admin_enqueue_scripts', 'coyote_edge_enqueue_meta_box_styles');
		add_action('admin_enqueue_scripts', 'coyote_edge_enqueue_meta_box_scripts');
	}

	add_action('add_meta_boxes', 'coyote_edge_meta_box_add');
}

if(!function_exists('coyote_edge_meta_box_save')) {
	/**
	 * Function that saves meta box to postmeta table
	 * @param $post_id int id of post that meta box is being saved
	 * @param $post WP_Post current post object
	 */
	function coyote_edge_meta_box_save( $post_id, $post ) {
		global $coyote_edge_Framework;
		global $coyote_edge_IconCollections;

		$nonces_array = array();
		$meta_boxes = coyote_edge_framework()->edgtMetaBoxes->getMetaBoxesByScope($post->post_type);
		...
		foreach ($coyote_edge_Framework->edgtMetaBoxes->options as $key=>$box ) {
	...
	add_action( 'save_post', 'coyote_edge_meta_box_save', 1, 2 );
}

if(!function_exists('coyote_edge_render_meta_box')) {
	/**
	 * Function that renders meta box
	 * @param $post WP_Post post object
	 * @param $metabox array array of current meta box parameters
	 */
	function coyote_edge_render_meta_box($post, $metabox) {?>
...
if(!function_exists('coyote_edge_meta_box_add_hidden_class')) {
	/**
	 * Function that adds class that will initially hide meta box
	 * @param array $classes array of classes
	 * @return array modified array of classes
	 */
	function coyote_edge_meta_box_add_hidden_class( $classes=array() ) {
...
if(!function_exists('coyote_edge_remove_default_custom_fields')) {
	/**
	 * Function that removes default WordPress custom fields interface
	 */
	function coyote_edge_remove_default_custom_fields() {
	...
	add_action('do_meta_boxes', 'coyote_edge_remove_default_custom_fields');
}


if(!function_exists('coyote_edge_get_custom_sidebars')) {
	/**
	 ...
	 */
	function coyote_edge_get_custom_sidebars() {
...
if(!function_exists('coyote_edge_generate_icon_pack_options')) {
    /**
     * Generates options HTML for each icon in given icon pack
     * Hooked to wp_ajax_update_admin_nav_icon_options action
     */
    function coyote_edge_generate_icon_pack_options() {
        global $coyote_edge_IconCollections;

        $html = '';
        $icon_pack = isset($_POST['icon_pack']) ? $_POST['icon_pack'] : '';
        $collections_object = $coyote_edge_IconCollections->getIconCollection($icon_pack);
    ...
    add_action('wp_ajax_update_admin_nav_icon_options', 'coyote_edge_generate_icon_pack_options');
}

if(!function_exists('coyote_edge_admin_notice')) {
    /**
     ...
     */
    function coyote_edge_admin_notice($id, $message, $class, $is_dismisable = true) {
        ...
        if(!$is_dismised && is_admin()) {
            ...
            if($is_dismisable) {
                echo '<strong style="display: block; margin-top: 7px;"><a href="'.esc_url(add_query_arg('edgt_dismis_notice', $id)).'">'.esc_html__('Dismiss this notice', 'coyote').'</a></strong>';
            }
...
if(!function_exists('coyote_edge_save_dismisable_notice')) {
    /**
     * Updates user meta with dismisable notice. Hooks to admin_init action
     * in order to check this on every page request in admin
     */
    function coyote_edge_save_dismisable_notice() {
    ...
    add_action('admin_init', 'coyote_edge_save_dismisable_notice');
}

if(!function_exists('coyote_edge_hook_twitter_request_ajax')) {
    /**
     ...
     */
    function coyote_edge_hook_twitter_request_ajax() {
        EdgefTwitterApi::getInstance()->obtainRequestToken();
    }

    add_action('wp_ajax_edgt_twitter_obtain_request_token', 'coyote_edge_hook_twitter_request_ajax');
}
