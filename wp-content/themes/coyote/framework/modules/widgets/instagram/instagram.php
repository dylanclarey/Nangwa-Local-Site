...
class CoyoteEdgeInstagramWidget extends CoyoteEdgeWidget {
    protected $params;
    public function __construct() {
        parent::__construct(
            'edgtf_instagram_widget',
            esc_html__('Edge Instagram Widget','coyote'),
            array( 'description' => esc_html__( 'Display instagram images', 'coyote' ) )
        );
    ...
    protected function setParams() {
        $this->params = array(
            ...
            array(
                'name' => 'tag',
                'type' => 'textfield',
                'title' => esc_html__('Tag', 'coyote'),
            ),
            array(
                'name' => 'number_of_photos',
                'type' => 'textfield',
                'title' => esc_html__('Number of photos', 'coyote'),
            ),
            array(
                'name' => 'number_of_cols',
                'type' => 'dropdown',
                'title' => esc_html__('Number of columns','coyote'),
                'options' => array(
                    '2' => esc_html__('Two','coyote'),
                    '3' => esc_html('Three','coyote'),
                    '4' => esc_html__('Four','coyote'),
                    '6' => esc_html__('Six','coyote'),
                    '9' => esc_html__('Nine','coyote')
                )
            ),
            array(
                'name' => 'image_size',
                'type' => 'dropdown',
                'title' => esc_html__('Image Size', 'coyote'),
                'options' => array(
                    'thumbnail' => esc_html__('Small', 'coyote'),
                    'low_resolution' => esc_html__('Medium', 'coyote'),
                    'standard_resolution' => esc_html__('Large', 'coyote'),
                )
            ),
            array(
                'name' => 'transient_time',
                'type' => 'textfield',
                'title' => esc_html__('Images Cache Time', 'coyote'),
            ),
        );
    }
            ...
            foreach ($images_array as $image) { ?>
                <li>
                    <a href="<?php echo esc_url($instagram_api->getHelper()->getImageLink($image)); ?>" target="_blank">
                        <?php echo coyote_edge_kses_img($instagram_api->getHelper()->getImageHTML($image, $image_size)); ?>
                    </a>
                </li>
            <?php } ?>
            </ul>
