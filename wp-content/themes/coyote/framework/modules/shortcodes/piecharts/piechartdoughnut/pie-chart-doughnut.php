<?php
namespace CoyoteEdge\Modules\Shortcodes\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Value ', 'coyote') . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'colorpicker',
				'heading' => esc_html__('Chart Color ', 'coyote') . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Legend ', 'coyote') . $i,
		...
		vc_map( array(
			'name' => esc_html__('Pie Chart 3 (Doughnut)', 'coyote'),
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				array(
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Height', 'coyote'),
						'param_name' => 'height',
					),
				),
				$chart_fields
	...
	public function render($atts, $content = null) {
		...
		$html = coyote_edge_get_shortcode_module_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
