<?php
namespace CoyoteEdge\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('coyote_edge_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('coyote_edge_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('coyote_edge_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        if(!empty($config['affect_slider'])) {

            add_filter('coyote_edge_slider_item_padding', array($this, 'sliderItemPaddingFilter'));
            add_filter('coyote_edge_slider_navigation_padding', array($this, 'sliderNavigationPaddingFilter'));
        }

        do_action('coyote_edge_after_header_connect');
    }
     ...
     *
     * Hooks to coyote_edge_title_content_padding filter
     *
     * @return int
     */
     ...
     *
     * Hooks to coyote_edge_slider_item_padding filter
     *
     * @return int
     */
