...
if(!function_exists('coyote_edge_single_portfolio')) {
    function coyote_edge_single_portfolio() {
        $portfolio_template = coyote_edge_get_portfolio_single_type();
        $holder_class = $portfolio_template;
        $fullwidth = false;
        $related_slug = '';

        if (coyote_edge_string_ends_with($portfolio_template,'-right')){
        	$holder_class = coyote_edge_filter_suffix($portfolio_template,'-right');
        }

        if (in_array($holder_class, array('full-width-custom','wide-slider','wide-images','split-screen','full-screen-slider'))){
        	$fullwidth = true;
        }

        if (in_array($holder_class, array('wide-slider','wide-images'))){
        	$related_slug = 'wide';
        }
        ...
        if (($portfolio_template == 'full-screen-slider') && (coyote_edge_options()->getOptionValue('portfolio_single_fullscreen_slider_scroll') == 'yes')) {
            $params['holder_class'][] = 'edgtf-change-on-scroll';
        }

        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'edgtf-portfolio-gallery-' . coyote_edge_options()->getOptionValue('portfolio_single_numb_columns');
        }

        coyote_edge_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('coyote_edge_get_portfolio_single_type')) {
    function coyote_edge_get_portfolio_single_type() {
        return coyote_edge_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('coyote_edge_get_portfolio_single_media')) {
    function coyote_edge_get_portfolio_single_media() {
	    ...
	    $single_type = coyote_edge_get_portfolio_single_type();
        if($image_ids !== '') {
	        ...
	        if( $single_type === 'small-masonry' || $single_type === 'big-masonry'){
		        foreach($image_ids as $image_id) {
			        ...
			        $media['skin'] = get_post_meta($image_id, '_slider_image_skin', true);
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'coyote_edge_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = coyote_edge_portfolio_get_video_url($video);
...
if(!function_exists('coyote_edge_portfolio_get_video_url')) {
    function coyote_edge_portfolio_get_video_url($video) {
...
if(!function_exists('coyote_edge_portfolio_get_media_html')) {
    function coyote_edge_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (coyote_edge_get_portfolio_single_type() == 'gallery') ? true : false;
        $params['full_screen_slider'] = (coyote_edge_get_portfolio_single_type() == 'full-screen-slider') ? true : false;

        if($media['type'] == 'image') {
			if(!$params['full_screen_slider']) {
	            $params['lightbox'] = coyote_edge_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
	        }
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
			if(!$params['full_screen_slider']) {
	            $params['lightbox'] = coyote_edge_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
	        }
        ...
        coyote_edge_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('coyote_edge_compare_portfolio_videos')) {
    /**
     ...
     */
    function coyote_edge_compare_portfolio_videos($a, $b) {
...
if(!function_exists('coyote_edge_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function coyote_edge_compare_portfolio_options($a, $b) {
...
if(!function_exists('coyote_edge_portfolio_get_info_part')) {
    function coyote_edge_portfolio_get_info_part($part) {
        $portfolio_template = coyote_edge_get_portfolio_single_type();

        coyote_edge_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template, array('type'=>$portfolio_template));
    }
}