...
if(!function_exists('coyote_edge_get_title')) {
    /**
     * Loads title area HTML
     */
    function coyote_edge_get_title() {
        $id = coyote_edge_get_page_id();

        extract(coyote_edge_title_area_height());
        extract(coyote_edge_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = coyote_edge_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_type_meta", true)) !== ""){
            ...
            $type = coyote_edge_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = coyote_edge_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }
		...
		if (coyote_edge_get_meta_field_intersect('title_enable_underscore', $id) == 'yes'){
        ...
        $parameters = array(
            ...
            'underscore_enabled' => $underscore_enabled
        );

        $parameters = apply_filters('coyote_edge_title_area_height_params', $parameters);

        coyote_edge_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('coyote_edge_get_title_text')) {
    /**
     * Function that returns current page title text. Defines coyote_edge_title_text filter
     ...
     */
    function coyote_edge_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'coyote');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'coyote') . " " . get_the_author();
        }
		...
		elseif (is_search()) {
			...
			$title = esc_html__('Search Results for', 'coyote').': '.get_search_query();
		}

        //is current page 404?
        elseif (is_404()) {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'coyote');
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(coyote_edge_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(coyote_edge_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','coyote');
        }
        ...
        $title = apply_filters('coyote_edge_title_text', $title);

        return $title;
    }
}

if(!function_exists('coyote_edge_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see coyote_edge_get_title_text()
     */
    function coyote_edge_title_text() {
        echo coyote_edge_get_title_text();
    }
}

if(!function_exists('coyote_edge_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function coyote_edge_subtitle_text() {
        $id = coyote_edge_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('coyote_edge_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see coyote_edge_is_product_category()
     * @see get_search_query()
     */
    function coyote_edge_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = coyote_edge_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="edgtf-delimiter" '.coyote_edge_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="edgtf-current" '.coyote_edge_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.coyote_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.coyote_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.coyote_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.coyote_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || coyote_edge_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'coyote') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.coyote_edge_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.coyote_edge_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.coyote_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.coyote_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','coyote') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','coyote') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'coyote') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('coyote_edge_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function coyote_edge_get_title_area_height() {
        ...
        if(($meta_temp = get_post_meta($id, 'edgtf_title_area_type_meta', true)) != '') {
            ...
            $title_type = coyote_edge_options()->getOptionValue('title_area_type');
        }

        $title_size = coyote_edge_get_meta_field_intersect('title_area_text_size', $id);

        $title_height = 160; // default title height without header height
        ...
        } elseif(coyote_edge_options()->getOptionValue('title_area_height') !== '') {
            $title_height = coyote_edge_options()->getOptionValue('title_area_height');
        }

        return apply_filters('coyote_edge_title_area_height', $title_height);
    }
}

if(!function_exists('coyote_edge_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function coyote_edge_get_title_content_padding() {
        return apply_filters('coyote_edge_title_content_padding', 0);
    }
}


if(!function_exists('coyote_edge_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function coyote_edge_title_area_height() {
        $id = coyote_edge_get_page_id();
        $title_height_and_padding = array();
        $title_height          = coyote_edge_get_title_area_height();
        $header_height_padding = coyote_edge_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = coyote_edge_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "edgtf_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = coyote_edge_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('coyote_edge_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function coyote_edge_title_area_background() {
        $id = coyote_edge_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = coyote_edge_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = coyote_edge_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = coyote_edge_get_image_dimensions($background_image);
